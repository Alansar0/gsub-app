<x-layouts.app>

<div class="bg-[#0d0d0f] text-white min-h-screen p-4 font-sans">

  <!-- Header -->
  <div class="flex items-center mb-6">
    <button class="text-gray-300 text-xl mr-3">←</button>
    <h1 class="text-lg font-semibold">Bank Transfer</h1>
  </div>

  <!-- Bank Cards -->
  <div class="space-y-4">

    <!-- Bank Palmpay -->
    <div class="border border-gray-700 rounded-xl p-4">
      <p class="text-gray-300 mb-2 font-medium">Bank Palmpay</p>
      <div class="flex items-center mb-2">
        <div class="flex items-center bg-blue-900 text-blue-200 px-3 py-1 rounded-lg text-sm font-semibold">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 16h8M8 12h8m-6 8h6a2 2 0 002-2V6a2 2 0 00-2-2h-6a2 2 0 00-2 2v12a2 2 0 002 2z" />
          </svg>
          6653258549
        </div>
      </div>
      <p class="text-gray-400 text-sm">Cool Data - AHMAD SAADU NUHU</p>
    </div>

    <!-- Bank Moniepoint -->
    <div class="border border-gray-700 rounded-xl p-4">
      <p class="text-gray-300 mb-2 font-medium">Bank Moniepoint</p>
      <div class="flex items-center mb-2">
        <div class="flex items-center bg-blue-900 text-blue-200 px-3 py-1 rounded-lg text-sm font-semibold">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 16h8M8 12h8m-6 8h6a2 2 0 002-2V6a2 2 0 00-2-2h-6a2 2 0 00-2 2v12a2 2 0 002 2z" />
          </svg>
          6365563708
        </div>
      </div>
      <p class="text-gray-400 text-sm">Cool Data - AHMAD SAADU NUHU</p>
    </div>

    <!-- Bank 9PSB -->
    <div class="border border-gray-700 rounded-xl p-4">
      <p class="text-gray-300 mb-2 font-medium">Bank 9PSB Bank</p>
      <div class="flex items-center mb-2">
        <div class="flex items-center bg-blue-900 text-blue-200 px-3 py-1 rounded-lg text-sm font-semibold">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 16h8M8 12h8m-6 8h6a2 2 0 002-2V6a2 2 0 00-2-2h-6a2 2 0 00-2 2v12a2 2 0 002 2z" />
          </svg>
          0
        </div>
      </div>
      <p class="text-gray-400 text-sm">Cool Data - AHMAD SAADU NUHU</p>
    </div>

    <!-- Bank Wema -->
    <div class="border border-gray-700 rounded-xl p-4">
      <p class="text-gray-300 mb-2 font-medium">Bank Wema</p>
      <div class="flex items-center mb-2">
        <div class="flex items-center bg-blue-900 text-blue-200 px-3 py-1 rounded-lg text-sm font-semibold">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 16h8M8 12h8m-6 8h6a2 2 0 002-2V6a2 2 0 00-2-2h-6a2 2 0 00-2 2v12a2 2 0 002 2z" />
          </svg>
          9747801635
        </div>
      </div>
      <p class="text-gray-400 text-sm">Cool Data - AHMAD SAADU NUHU</p>
    </div>

    <!-- Bank Sterling -->
    <div class="border border-gray-700 rounded-xl p-4">
      <p class="text-gray-300 mb-2 font-medium">Bank Sterling</p>
      <div class="flex items-center mb-2">
        <div class="flex items-center bg-blue-900 text-blue-200 px-3 py-1 rounded-lg text-sm font-semibold">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 16h8M8 12h8m-6 8h6a2 2 0 002-2V6a2 2 0 00-2-2h-6a2 2 0 00-2 2v12a2 2 0 002 2z" />
          </svg>
          9987120937
        </div>
      </div>
      <p class="text-gray-400 text-sm">Cool Data - AHMAD SAADU NUHU</p>
    </div>

  </div>

</div>

</x-layouts.app>

