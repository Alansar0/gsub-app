<x-layouts.app>
    <div class="min-h-screen flex flex-col items-center justify-center bg-[#0C141C] text-white px-4 py-10">
        ...
        <!-- Circular Artwork -->
        <div
            class="relative w-64 h-64 rounded-full bg-gradient-to-br from-[#0D2C35] to-[#112C3A] flex items-center justify-center shadow-[0_0_40px_rgba(0,255,209,0.3)]">
            ...
            <svg class="absolute top-0 left-0 w-full h-full" viewBox="0 0 100 100">
                <circle cx="50" cy="50" r="48" stroke="#00FFD1" stroke-width="1.6" fill="none"
                    opacity="0.25" />
                <circle id="progressCircle" cx="50" cy="50" r="48" stroke="#00FFD1" stroke-width="2.5"
                    fill="none" stroke-dasharray="301.59" stroke-dashoffset="301.59" stroke-linecap="round"
                    class="transition-all duration-200 ease-linear" />
            </svg>

            <!-- Play/Pause Button -->
            <button id="playBtn"
                class="absolute bottom-6 right-6 w-16 h-16 bg-[#00FFD1] rounded-full flex items-center justify-center shadow-[0_0_25px_rgba(0,255,209,0.6)] hover:scale-110 transition">
                            ...
                            @foreach ($quiz['options'] as $i => $opt)
                                <label class="block">
                                    <input type="radio" name="quiz{{ $index }}" value="{{ $i }}"
                                        required class="mr-2">{{ $opt }}</label>
                            @endforeach

                        </div>
                    @endforeach
                    <button type="submit"
                        class="w-full bg-[#00FFD1] text-[#0C141C] py-2 rounded-lg mt-3 font-semibold hover:scale-105 transition">Submit
                        Answers
                    </button>
                </form>
        </div>
